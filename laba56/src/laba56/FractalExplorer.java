...
public class FractalExplorer
{
    ...
    public void createAndShowGUI()
    {
        ...
        // регистрируем экземпляр обработчика Mouse Listener в фрейме, метод извлекает слои панели содержимого для добавления объекта
        frame.pack();
        frame.setVisible(true);
        frame.setResizable(false);
    }
    private class FractalHandler implements ActionListener {
        // обработчик событий кнопок и поддержка списка фракталов
        public void actionPerformed(ActionEvent e) {
            String cmd = e.getActionCommand();
            if (e.getSource() == fractalChooser) {
                String selectedItem = fractalChooser.getSelectedItem().toString();
                // выбор из трех фракталов
                ...
                //строка гарантирует, что средство выбора
                // не разрешит пользователю использование отличных от png форматов.
                if (chooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) { // открывает
                    ...
                    try {
                        ...
                        //Если пользователь уже выбрал директорию для сохранения
    ...
    private class FractalWorker extends SwingWorker<Object, Object>
            // SwingWorker - абстрактный класс, включающий методы doInBackground() и done()
    { // FractalWorker отвечает за вычисление значений цвета для  одной строки фрактала
        ...
        public Object doInBackground() // метод, который  выполняет фоновые операции
        {
            ...
            for (int x = 0; x < displaySize; x++) {
                // цикл должен сохраняет каждое значение RGB в соответствующем элементе целочисленного массива
                double xCoord = FractalGenerator.getCoord(range.x, range.x + range.width,
                        displaySize, x);
                int numIters;
                int rgbColor = 0;
        ...
        // Методу разрешено взаимодействовать с пользовательским интерфейсом
        {
            for (int x = 0; x < displaySize; x++) {
